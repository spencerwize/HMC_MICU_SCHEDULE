# ─────────────────────────────────────────────────────────────────────────────
# new_parse_time_off.R  —  Text-value parser supporting Google Sheets,
...
#   Then call new_parse_time_off(path) wherever parse_time_off() was called.
#
# Returns: named list  person → data.frame(date = Date, type = chr)
#   type values: "off" | "vac" | "cme"
# ─────────────────────────────────────────────────────────────────────────────

new_parse_time_off <- function(path, sheet = NULL) {
...
gs4_auth_auto <- function() {
  ...
  if (nzchar(svc_json)) {
    # Could be a file path or the raw JSON string
    cred <- if (file.exists(svc_json)) svc_json else
              jsonlite::fromJSON(svc_json)   # gargle accepts a list too
  ...
  # Fall back: public / published sheet (no credentials)
